...
{role_context}
A backend software engineer with 1–2 years of experience in Golang and Docker is expected to work on small-scale service development and deployment workflows. Their main responsibility is to understand the existing application structure, implement new features or fix minor bugs in Golang code, and use Docker for containerizing and running applications locally. They should be able to write simple Golang classes, handle basic REST endpoints, and build Docker images using Dockerfiles. The candidate is not required to handle complex CI/CD pipelines or advanced multi-container orchestration but should understand basic containerization concepts, image building, and running containers efficiently.
...
- All dependencies properly configured in go.mod/go.sum
- Pre-configured environment variables and configuration ready for containerization
...
- Provide a FULLY WORKING Golang application that implements the business logic from the input scenario, requiring only basic Docker setup
- **GOLANG APPLICATION**: Complete and OPTIMIZED implementation of the scenario's requirements - requires ZERO Golang code changes
...
- **MINIMAL CONFIGURATION (15%)**: Only environment-specific values in configuration files (database URLs, ports, service names) - NO code changes
...
- Generate complete Golang application implementing the scenario's requirements with basic Docker setup requiring fundamental Docker knowledge
- Task complexity aligns with basic proficiency (1-2 years Docker experience):

  **Docker Focus Areas (85% of task):**
  - Understanding and using official Golang base images correctly
  ...
  - Basic multi-stage build concepts (builder vs runtime)
  
  **Minimal Configuration (15% of task):**
  - Updating connection strings in config files for containerized services
...
- **CRITICAL**: Golang application is COMPLETE, FUNCTIONAL, FULLY OPTIMIZED implementation of the scenario
- All REST API endpoints implementing the scenario's API requirements are working
- Basic Docker configuration requiring fundamental Docker knowledge
- **TASK FOCUS**: Docker containerization (85%), NOT Golang development (15% config only)
...
1. **Go Module Configuration:**
   - go.mod MUST include ALL required dependencies with correct versions
   - MUST use stable Go version (1.19+ recommended)
   - Dependencies MUST match what Docker build process expects AND what the scenario requires (e.g., GORM for database scenarios, go-redis for caching scenarios)
   - All dependency versions MUST be compatible and tested
   - NO dependency conflicts or missing modules
   - go.sum MUST be properly generated with checksums

2. **Dockerfile Build Strategy:**
   - **CRITICAL**: Dockerfile MUST use official golang image for building
   - Use multi-stage build: Build stage → Runtime stage
   - Build stage: Use golang:1.21-alpine or similar, copy go.mod + go.sum, download deps, copy source, build binary
   - Runtime stage: Use alpine:latest or distroless, copy binary from build stage
   - Set proper CGO_ENABLED flag (0 for static binary)
   - Use proper build flags: -ldflags="-w -s" for size optimization
   - NEVER assume local Go installation
...
3. **Docker-Compose Configuration:**
   ...
   - Port mappings MUST match application configuration
...
4. **Application Configuration Consistency:**
   - Configuration files (config.yaml, .env, or constants) database URLs MUST match docker-compose service names
   - Port numbers MUST be consistent across:
     - Application configuration (server port)
     - Dockerfile EXPOSE directive
     - docker-compose ports mapping
   - Database credentials MUST match between application config and docker-compose
...
5. **File System and Permissions:**
   ...
   - Binary naming in Dockerfile MUST match build output
   - No permission issues - proper WORKDIR usage
   - Volume mount paths properly configured
   - Binary should be executable in runtime stage
...
- Uses official Golang Docker image for building (NOT local installation)
- May use basic multi-stage build (candidate can optimize)
- Basic base image selection (candidate can optimize to distroless or scratch)
...
- Binary built with basic flags (candidate can optimize with build flags)
- **CRITICAL**: Must build successfully without requiring local Go installation
...
- Golang code fully optimized and functional, implementing the scenario's business requirements
- **CRITICAL**: Golang files are COMPLETE - NO changes needed
- Docker files basic but functional
- **PRIMARY FOCUS**: 85% Docker work, 15% configuration
- NO TODO comments in Golang code
...
- **SCENARIO IMPLEMENTATION**: All handlers, services, models, repositories implement the scenario's requirements
- Follow idiomatic Go patterns: proper error handling, context usage, graceful shutdown

## .gitignore INSTRUCTIONS:
Comprehensive file including:
- Go artifacts: vendor/, *.exe, *.exe~, *.dll, *.so, *.dylib, *.test, *.out, bin/, dist/
- IDE files: .idea/, .vscode/, *.swp, *.swo, *~
- Build artifacts: main, app, server (common binary names)
- Logs: *.log, logs/
- Docker: .docker/
- OS: .DS_Store, Thumbs.db, desktop.ini
- Environment: .env, .env.local
- Other: *.tmp, *.bak, coverage.txt, coverage.html

## .dockerignore INSTRUCTIONS:
May be missing or basic - candidate creates/improves:
- Should exclude: .git/, .idea/, *.md (except needed), .gitignore, *.log, vendor/ (if not using vendoring)
- Keep: go.mod, go.sum, main.go, internal/, cmd/, pkg/, config files

## README.md STRUCTURE:

### Task Overview (MANDATORY - 2-3 substantial sentences)
**CRITICAL**: This section MUST describe the specific business scenario from input_scenarios and explain why Docker containerization is needed for this particular use case. Explain the Golang application is fully functional and focus is on Docker setup. Connect the scenario's business problem to the containerization challenge. NEVER empty content.

Example: "An e-commerce platform has developed a product catalog service that manages inventory and pricing using Golang. The service needs to be containerized for deployment across multiple environments while ensuring efficient resource usage and quick startup times."
...
- "Think about how Go modules can be cached in Docker layers"
...
- "Consider the benefits of multi-stage builds for Go applications"
- "Think about static vs dynamic linking for Go binaries"
...
- "Optimize binary size and container image"
...
- "Verify binary size and container layers"
...
{{
   ...
   "question": "Brief description incorporating the scenario's business problem and what Docker setup needs to be understood and configured. Focus on Docker fundamentals for the given scenario, NOT Golang changes.",
   "code_files": {{
      ...
      ".gitignore": "Comprehensive exclusions for Go, Docker, IDE",
      ".dockerignore": "Basic file or missing - candidate improves",
      "go.mod": "Complete Go modules file with ALL dependencies required by the scenario, correct versions",
      "go.sum": "Complete checksums file for all dependencies",
      ...
      "Dockerfile": "Basic Dockerfile using Golang Docker image for building, functional but simple multi-stage build",
      "run.sh": "Complete deployment script with health checks",
      "kill.sh": "Complete cleanup script",
      "main.go": "Complete main entry point with server setup and graceful shutdown",
      "internal/handlers/*.go": "Complete HTTP handlers implementing scenario's API",
      "internal/services/*.go": "Complete service layer implementing scenario's business logic",
      "internal/models/*.go": "Complete structs matching scenario's domain",
      "internal/repository/*.go": "Complete repositories for scenario's data access",
