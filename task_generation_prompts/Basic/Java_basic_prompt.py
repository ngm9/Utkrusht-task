...
Practical guidance without revealing implementations:
  ...
  - Point toward considering what happens when things go wrong and how to communicate errors effectively
  - Hint at exploring how to validate incoming data before processing it
  - Recommend considering how configuration values can be externalized and managed
  - Suggest analyzing how to structure responses that clients can easily understand and consume
  - Mention thinking about what annotations Spring provides to mark different component types
...
2. **name** must be short, descriptive, kebab-case (e.g., "library-borrow-return-fix")
...
6. **outcomes**, **short_overview**, **pre_requisites** must be bullet-point lists in simple language
7. **hints** must be a single line; **definitions** must include relevant Java/Spring terms
